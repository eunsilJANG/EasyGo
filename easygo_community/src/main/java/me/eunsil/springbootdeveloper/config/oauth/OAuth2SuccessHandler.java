...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    public static final String REFRESH_TOKEN_COOKE_NAME = "refresh_token";
    public static final Duration REFRESH_TOKEN_DURATION = Duration.ofDays(14);
    public static final Duration ACCESS_TOKEN_DURATION = Duration.ofDays(1);
    private static final String NICKNAME_REDIRECT_PATH = "http://localhost:5173/set-nickname";
...
public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
    ...
    // 리프레시 토큰이 없는 경우 = 최초 로그인으로 판단
    boolean isFirstLogin = !refreshTokenRepository.findByUserId(user.getId()).isPresent();
    ...
    String targetUrl = UriComponentsBuilder
            .fromUriString(isFirstLogin ? NICKNAME_REDIRECT_PATH : PREFERENCES_REDIRECT_PATH)
    ...
    private void addRefreshTokenToCookie(HttpServletRequest request, HttpServletResponse response, String refreshToken){
        ...
        CookieUtil.deleteCookie(request, response, REFRESH_TOKEN_COOKE_NAME);
        CookieUtil.addCookie(response, REFRESH_TOKEN_COOKE_NAME, refreshToken, cookieMaxAge);
    }
...
//    // 엑세스 토큰을 패스에 추가하고 닉네임 상태에 따라 리다이렉트 경로 결정
